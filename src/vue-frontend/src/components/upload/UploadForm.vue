...
import { uploadPdf } from '@/api/requst';
const { t } = useI18n();
...
<template>
  <VCard width="940px">
    <VToolbar tag="div">
      ...
      <VToolbarTitle><VIcon icon= "mdi-file-upload-outline"/>{{ t('pdf_file_upload') }}</VToolbarTitle>
    ...
    <VCardActions>
      <VBtn @click="uploadFile" type="submit" variant="elevated" color="primary">{{ t('submit') }}</VBtn>
      <VBtn @click="resetFile" type="reset" variant="tonal">{{ t('reset') }}</VBtn>
    </VCardActions>
